export default class ScrollProgress {
  ...
  setInfos() {
    ...
    for (let i = 0; i < this.elementsLength; i++) {
      ...
      this.elementsInfo[i].offset =
        this.elements[i].getAttribute('data-offset') !== null
      ...
      this.elementsInfo[i].translateZ =
        Number(this.elements[i].getAttribute('data-translate-z')) || this.options.translateZ
      this.elementsInfo[i].translateUnit =
        this.elements[i].getAttribute('data-translate-unit') || this.options.translateUnit
      this.elementsInfo[i].skewX =
        Number(this.elements[i].getAttribute('data-skew-x')) || this.options.skewX
      this.elementsInfo[i].skewY =
        Number(this.elements[i].getAttribute('data-skew-y')) || this.options.skewY
      this.elementsInfo[i].rotate =
        Number(this.elements[i].getAttribute('data-rotate')) || this.options.rotate
      this.elementsInfo[i].blur =
        Number(this.elements[i].getAttribute('data-blur')) || this.options.blur
      //for scale & opacity we need to check typeof otherwise it would return false for "0"
      this.elementsInfo[i].scale =
        this.elements[i].getAttribute('data-scale') !== null
          ? Number(this.elements[i].getAttribute('data-scale'))
          : this.options.scale
      this.elementsInfo[i].opacity =
        this.elements[i].getAttribute('data-opacity') !== null
          ? Number(this.elements[i].getAttribute('data-opacity'))
          : this.options.opacity
      this.elementsInfo[i].easing =
        this.elements[i].getAttribute('data-easing') || this.options.easing
  ...
  setStaggers() {
    ...
    for (let i = 0; i < containersLength; i++) {
      ...
      const offset =
        this.staggerContainers[i].getAttribute('data-offset') !== null
  ...
  applyEffect(all = false) {
    ...
    for (let i = 0; i < this.elementsLength; i++) {
      ...
      let isInView =
        scrollY + this.windowHeight + 200 > info.top && scrollY < info.top + this.windowHeight

      if (isInView || all === true) {
        let factor =
          (top + this.windowHeight - info.offset * this.windowHeight) /
